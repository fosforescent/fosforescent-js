...
import { RootFosInterpreter } from ".";
describe('interpreter basics', () => {
    test('spawn, getStack work properly', () => {
        ...
        const newTaskInterpreter2 = newTestInt2.getChildren().filter(child => child.getInstruction() === store.allOfAddress && child.getTarget() === newNode2.getAddress())[0];
    ...
    test('returns correctly and updates root when creating task', () => {
        const store = new Store();
        const interpreter = new RootFosInterpreter(store, store.getNodeByAddress(store.voidAddress), store.getNodeByAddress(store.unitAddress));
