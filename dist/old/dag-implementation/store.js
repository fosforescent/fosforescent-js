...
(function (NodeType) {
    ...
    NodeType[NodeType["Internal"] = 1] = "Internal";
    NodeType[NodeType["External"] = 2] = "External";
})(NodeType || (NodeType = {}));
export class Store {
    table = new Map();
    ...
    constructor({ json, publicKey } = {}) {
        if (json) {
            this.hashFunc = (item) => this.hashString(JSON.stringify(item.getValue().toString()));
            this.fromJSON(json);
    ...
    updateRoot(oldInterpreter, newInterpreter) {
    ...
    // }
    getRootFromStubString(stubString) {
        const re = /\(([\w]{10})\)([\w]{10})/;
    ...
    checkAddress(address, externalAllowed = true) {
    ...
    insertExternal(data) {
        ...
        if (this.externalData.get(address))
            return address;
        else {
    ...
    getNodeByAddress(address) {
        ...
        if (this.externalData.has(address))
            return this.externalData.get(address);
        const edges = this.table.get(address);
        ...
        if (!result)
            throw new Error(`Unable to construct node for address ${address}`);
        return result;
    }
    query(query) {
        /**
    ...
    matchPattern(pattern, entry) {
        ...
        for (const [patternKey, patternValues] of patternMap.entries()) {
            if (!nodeMap.has(patternKey)) {
                ...
                const entryTargetsForKey = nodeMap.get(patternKey);
                if (patternValues.length !== entryTargetsForKey.length) {
                    ...
                    patternValues.forEach((patternValue, index) => {
                        if (patternValue === this.unitAddress) {
                            patternResult.push(this.getNodeByAddress(entryTargetsForKey[index]));
                        }
                        else if (entryTargetsForKey[index] !== patternValue) {
                            // TODO: figure out what do here
                            const subQueryResults = this.matchPattern(this.getNodeByAddress(patternValue), this.getNodeByAddress(entryTargetsForKey[index]));
    ...
    queryTriple(subject, predicate, object) {
        ...
        const tripleResults = subjectMatches.flatMap((subjectMatch) => {
            ...
            return allMatches.map(([predicateKey, objectKey]) => [subjectMatch, this.getNodeByAddress(predicateKey), this.getNodeByAddress(objectKey)]);
        });
        return tripleResults;
    }
    negativeQueryTriple(subject, predicate, object) {
        if (subject.getAddress() === this.voidAddress) {
            const nodesToTest = this.query(object);
            const result = nodesToTest.filter((nodeToTest) => {
                const test = this.queryTriple(this.getNodeByAddress(this.unitAddress), predicate, nodeToTest);
                return test.length === 0;
            });
            return result.map((node) => [subject, predicate, node]);
        }
    ...
    fromJSON(json) {
        try {
            ...
            const externalDataEntries = data.externalData.forEach((value) => {
            ...
            // console.log('gettingInt', this.externalData)
            const tableEntries = data.table.map(([key, value]) => {
                // console.log('table entry', key, value)
                return [key, value];
            });
            this.table = new Map(tableEntries);
            // console.log('gettingInt', this.externalData)
            this.init();
